...
# def allocate_flexible_participants_fully(strict_groups, flexible_participants, regrouping_map, group_size=3):
#     """
#     Allocate flexible participants to strict groups considering multiple criteria,
#     using the regrouping map to break down flexible options into stricter categories.
    
#     Parameters:
#     strict_groups (dict): A dictionary of strict group combinations with empty DataFrames.
#     flexible_participants (pd.DataFrame): DataFrame of flexible participants.
#     regrouping_map (dict): Dictionary that maps flexible options to stricter options.
#     group_size (int): Size of each group.
    
#     Returns:
#     dict: A dictionary of strict group combinations with allocated participants as DataFrames.
#     """
#     final_groups = strict_groups.copy()
...
#         # Allocate participant to all matching strict groups
#         allocated = False
#         for strict_combination in itertools.product(possible_freqs, possible_days, possible_times):
#             group_key = strict_combination
            
#             # Ensure that the group is a DataFrame
#             if not isinstance(final_groups[group_key], pd.DataFrame):
#                 final_groups[group_key] = pd.DataFrame()  # Convert to DataFrame if necessary
            
#             # Check the size of the group and concatenate
#             if len(final_groups[group_key]) < group_size:
#                 final_groups[group_key] = pd.concat([final_groups[group_key], participant.to_frame().T], ignore_index=True)
#                 allocated = True
#                 break  # Participant is assigned, no need to assign to multiple groups
        
#         # If participant couldn't be allocated, they could be assigned to a fallback group (optional)
#         if not allocated:
#             # Handle any fallback logic if needed (e.g., putting them in a separate group)
#             pass

#     return final_groups

# # Helper to initialize the strict groups as empty DataFrames
# def initialize_strict_groups(strict_combinations):
#     return {combination: pd.DataFrame() for combination in strict_combinations}


# def prepare_grouping_data(df):
#     """
#     Prepares data for grouping by extracting relevant columns, generating strict combinations,
#     initializing strict groups, and separating strict and flexible participants.
    
#     Parameters:
#     df (pd.DataFrame): The input DataFrame containing participant data.
    
#     Returns:
#     dict: A dictionary of initialized strict groups.
#     pd.DataFrame: DataFrame of strict participants.
#     pd.DataFrame: DataFrame of flexible participants.
#     """
...
#     regrouping_map = {
#         'I’m open to either': strict_frequencies,
...
#     # Initialize strict groups as empty lists
#     strict_groups = {combination: [] for combination in strict_combinations}

#     # Separate strict and flexible participants
#     strict_participants = df[
#         (df['H'].isin(strict_frequencies)) & 
#         (df['I'].isin(strict_days)) & 
#         (df['J'].isin(strict_times))
#     ]
    
#     flexible_participants = df[~df.index.isin(strict_participants.index)]

#     return strict_groups, strict_participants, flexible_participants, regrouping_map
